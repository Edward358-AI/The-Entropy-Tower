...
export const breakDownGoal = async (goal) => {
  ...
  const prompt = `
    ...
    - xp: 20-40 based on difficulty. Most tasks should be 25-35 XP. Only give 40 for genuinely hard tasks.
    - deadlineOffset: Days from now (1-7)
    
    Return ONLY a valid JSON array. No markdown, no "json" label.
    Example:
    [
      { "title": "Read Chapter 1", "xp": 25, "deadlineOffset": 1 },
      { "title": "Write outline for essay", "xp": 30, "deadlineOffset": 2 }
    ]
  `
