 ...
 *
 * Server-authoritative quest completion using Firestore transaction:
 ...
 * 6. Recomputes streak
 * 7. Writes all changes atomically via transaction
 *
...
async function handleCompleteQuest(db, userId, questId) {
  ...
  const result = await db.runTransaction(async (t) => {
    ...
    const [questSnap, statsSnap, heatmapSnap] = await Promise.all([
      t.get(questRef),
      t.get(statsRef),
      t.get(heatmapRef),
    ])
